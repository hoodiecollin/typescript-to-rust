/**
 * Specs for series 100 — I/O via the `@t2r/std` shim lane (issue #65, epic #52).
 *
...
defineDifferential("std-io", [
  // ── Sync filesystem round-trips (T2R_TMP) ─────────────────────────────────
  {
    ...
    src: `import { env, writeFile, readFile } from "@t2r/std";
...
console.log(readFile(p));`,
  ...
  {
    name: "IO3 appendFile",
    src: `import { env, writeFile, appendFile, readFile } from "@t2r/std";
...
console.log(readFile(p));`,
  ...
  },
  {
    ...
    src: `import { env, writeFile, exists } from "@t2r/std";
...
console.log(exists(p), exists(p + ".nope"));`,
  ...
  },
  {
    ...
    src: `import { env, writeFile, removeFile, exists } from "@t2r/std";
...
console.log(exists(p));`,
  ...
  },
  {
    ...
    src: `import { env, mkdir, writeFile, readDir } from "@t2r/std";
...
console.log(readDir(dir).join(","));`,
  ...
  // ── Sync fallible — missing path → throw / Err ────────────────────────────
  {
    ...
    src: `import { env, readFile } from "@t2r/std";
...
console.log(env("T2R_NOPE_UNSET") ?? "none");`,
  ...
  },
  {
    ...
    src: `import { readStdin } from "@t2r/std";
console.log(readStdin());`,
  ...
  },
  {
    ...
    src: `import { readLine } from "@t2r/std";
let l = readLine();
while (l !== null) {
  console.log(l);
  l = readLine();
  ...
  // ── Standard-stream Writer handle — byte-precise stdout ────────────────────
  {
    ...
    src: `import { stdout } from "@t2r/std";
const w = stdout();
w.write("ab");
w.write("cd");`,
    expected: "abcd",
  ...
  },
  {
    ...
    src: `import { stderr } from "@t2r/std";
stderr().writeLine("e");`,
    expected: "",
    extra: ({ rust }) => {
  ...
  // ── Async filesystem — tokio::fs ──────────────────────────────────────────
  {
    ...
    src: `import { env, fsAsync } from "@t2r/std";
...
console.log(xs.join(","));`,
  ...
  // ── Network HTTP — differential via the loopback server ───────────────────
  {
    ...
    src: `import { env, http } from "@t2r/std";
...
console.log(res.status);`,
  ...
  // ── Recognition — routes by specifier, not name ───────────────────────────
  {
    ...
    src: `import { env, readFile as rf, writeFile as wf } from "@t2r/std";
...
describe("100 fail-loud — out-of-surface I/O", () => {
  test("IO-FL7 an un-awaited async-I/O future is fail-loud", () => {
    expect(() =>
      compile(`import { http } from "@t2r/std";
...
http.get("http://x/");`),
  ...
  test("IO18 catching an async I/O error (await in try/catch) is deferred", () => {
    expect(() =>
      compile(`import { env, fsAsync } from "@t2r/std";
const missing: string = (env("T2R_TMP") ?? "") + "/nope.txt";
try {
...
const res = await http.put("http://x/");`),
  ...
  test("IO-FL13 an unknown Writer method is fail-loud", () => {
    expect(() =>
      compile(`import { stdout } from "@t2r/std";
const w = stdout();
w.frob("x");`),
    ).toThrow(/Writer|write.*writeLine.*flush/);
  });
  test("IO-FL14 an unknown @t2r/std import name is fail-loud", () => {
    expect(() =>
      compile(`import { readSocket } from "@t2r/std";`),
  ...
  test("IO-FL9/10/11 streaming/watch/socket names are not exported", () => {
    for (const name of ["createReadStream", "watch", "connect"]) {
      expect(() =>
        compile(`import { ${name} } from "@t2r/std";`),
      ).toThrow(/not exported/);
    }
  });
});
