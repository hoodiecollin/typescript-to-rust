...
describe("071.1 behavioral-interface trait synthesis + implements", () => {
  ...
  test("BINT8 object literal typed as a behavioral interface → fail-loud", () => {
    expect(() =>
      compile(`${SHAPE}\nconst s: Shape = { area: () => 5 };\nconsole.log(s.area());`),
    ).toThrow(UnsupportedError);
  });

  test("BINT9 heterogeneous behavioral-interface array → fail-loud", () => {
    expect(() =>
      compile(
        `${SHAPE}\n${CIRCLE}\n${SQUARE}\nconst xs: Shape[] = [new Circle(1), new Square(2)];\nconsole.log(xs.length);`,
      ),
    ).toThrow(UnsupportedError);
  });

  test("BINT10 `implements` a pure-data interface → fail-loud", () => {
    expect(() =>
      compile(
        `interface PureData { x: number; y: number }\nclass P implements PureData {\n  x: number; y: number;\n  constructor() { this.x = 1; this.y = 2; }\n}\nconsole.log(new P().x);`,
      ),
    ).toThrow(UnsupportedError);
  });
