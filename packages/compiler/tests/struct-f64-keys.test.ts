...
/** Differential: Rust run == TS-via-Bun == expected (shared-reference keys only). */
...
describe("074 struct Map/Set keys with f64 fields", () => {
  test("F64K1 `Map<Point, V>` set/get/has/delete/size (shared ref, differential)", async () => {
...
console.log(s.size);`;
  ...
  test("F64K5 arithmetic on the raw f64 field is untouched (differential)", async () => {
    const src = `interface Point { x: number; y: number; }
const m: Map<Point, number> = new Map<Point, number>();
...
console.log(s.size);`;
  ...
  test("F64K7 `Map<Point,V>` iteration unwraps the key newtype (differential)", async () => {
    const src = `interface Point { x: number; y: number; }
const m: Map<Point, string> = new Map<Point, string>();
const a: Point = { x: 1.5, y: 2.5 };
...
for (const [k, v] of m) { console.log(k.x, k.y, v); }`;
    await behaves(src, "1.5 2.5 a\n9 8 b");
    expect(compile(src)).toContain("for (PointKey(k), v) in m.iter()");
  });

  test("F64K8 `Set<Point>` iteration unwraps the newtype element (differential)", async () => {
    const src = `interface Point { x: number; y: number; }
const s: Set<Point> = new Set<Point>();
const a: Point = { x: 1.5, y: 2.5 };
s.add(a);
for (const p of s) { console.log(p.x, p.y); }`;
    await behaves(src, "1.5 2.5");
    expect(compile(src)).toContain("for PointKey(p) in s.iter()");
  });
