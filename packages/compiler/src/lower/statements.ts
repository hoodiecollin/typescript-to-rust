 ...
 * (`lowerTyped` / struct + interface literals), class-field planning, and the
 * shared expression-typing predicates (`receiverTypeOf` / `isStringExpr` /
 * `optionExprType` / `truthyCond` / …) the expression and type lowerers lean on.
 * The discriminated-union / `typeof` / `in` narrowing recognizers `lowerSwitch`
 * and `lowerIf` delegate to live in `./narrowing`.
 *
