 ...
 * statement-family — if/for/for-of/while, the switch-lowering + discriminated /
 * typeof / `in` narrowing recognizers, block lowering, `lowerVarDecl` (the largest
 * single lowerer), the typed-literal path (`lowerTyped` / struct + interface
 * literals), class-field planning, and the shared expression-typing predicates
 * (`receiverTypeOf` / `isStringExpr` / `optionExprType` / `truthyCond` / …) the
...
import { lowerThrow, lowerTry, rewriteFieldReads } from "./try-carrier";
import {
...
import { isAstNode, isNullishExpr, peelNonNull } from "./utils";
